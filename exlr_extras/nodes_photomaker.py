...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim, use_residual=True, operations=exlr.ops):
        super().__init__()
        if use_residual:
...
class PhotoMakerIDEncoder(exlr.clip_model.CLIPVisionModelProjection):
    def __init__(self):
        self.load_device = exlr.model_management.text_encoder_device()
        offload_device = exlr.model_management.text_encoder_offload_device()
        dtype = exlr.model_management.text_encoder_dtype(self.load_device)

        super().__init__(VISION_CONFIG_DICT, dtype, offload_device, exlr.ops.manual_cast)
        self.visual_projection_2 = exlr.ops.manual_cast.Linear(1024, 1280, bias=False)
        self.fuse_module = FuseModule(2048, exlr.ops.manual_cast)
...
class PhotoMakerLoader:
    ...
    def load_photomaker_model(self, photomaker_model_name):
        ...
        data = exlr.utils.load_torch_file(photomaker_model_path, safe_load=True)
...
class PhotoMakerEncode:
    ...
    def apply_photomaker(self, photomaker, image, clip, text):
        ...
        pixel_values = exlr.clip_vision.clip_preprocess(image.to(photomaker.load_device)).float()
        try:
