...
(function(global) {
    ...
    var HeavyGraph = (global.HeavyGraph = {
        VERSION: 0.4,
        ...
        registerNodeType: function(type, base_class) {
                    ...
                    set: function(v) {
                        switch (v) {
                            case "default":
                                ...
                                break;
                            case "box":
                                this._shape = HeavyGraph.BOX_SHAPE;
                                break;
                            case "round":
                                this._shape = HeavyGraph.ROUND_SHAPE;
                                break;
                            case "circle":
                                this._shape = HeavyGraph.CIRCLE_SHAPE;
                                break;
                            case "card":
                                this._shape = HeavyGraph.CARD_SHAPE;
                                break;
                            default:
                                this._shape = v;
                        }
            ...
            if (HeavyGraph.onNodeTypeRegistered) {
                HeavyGraph.onNodeTypeRegistered(type, base_class);
            }
            if (prev && HeavyGraph.onNodeTypeReplaced) {
                HeavyGraph.onNodeTypeReplaced(type, base_class, prev);
            }

            //warnings
            if (base_class.prototype.onPropertyChange) {
                console.warn(
                    "HeavyGraph node class " +
                        type +
        ...
        */
        ...
        wrapFunctionAsNode: function(
            ...
            var code = "";
            var names = HeavyGraph.getParameterNames(func);
        ...
        createNode: function(type, title, options) {
            ...
            if (HeavyGraph.catch_exceptions) {
                try {
            ...
            if (!node.pos) {
                node.pos = HeavyGraph.DEFAULT_POSITION.concat();
            }
            if (!node.mode) {
                node.mode = HeavyGraph.ALWAYS;
            }
        ...
        isValidConnection: function(type_a, type_b) {
            ...
            if (
                ...
                || (type_a == HeavyGraph.EVENT && type_b == HeavyGraph.ACTION)
            ) {
                return true;
            }
		...
		fetchFile: function( url, type, on_complete, on_error ) {
			...
			{
				if (url.substr(0, 4) == "http" && HeavyGraph.proxy) {
					url = HeavyGraph.proxy + url.substr(url.indexOf(":") + 3);
				}
				return fetch(url)
    ...
    if (typeof performance != "undefined") {
        HeavyGraph.getTime = performance.now.bind(performance);
    } else if (typeof Date != "undefined" && Date.now) {
        HeavyGraph.getTime = Date.now.bind(Date);
    } else if (typeof process != "undefined") {
        HeavyGraph.getTime = function() {
            var t = process.hrtime();
            return t[0] * 0.001 + t[1] * 1e-6;
        };
    } else {
        HeavyGraph.getTime = function getTime() {
    ...
    global.LGraph = HeavyGraph.LGraph = LGraph;
    ...
    LGraph.prototype.start = function(interval) {
        ...
        //launch
        this.starttime = HeavyGraph.getTime();
        ...
        if (do_not_catch_errors) {
            ...
            for (var i = 0; i < num; i++) {
                for (var j = 0; j < limit; ++j) {
                    var node = nodes[j];
                    if (node.mode == HeavyGraph.ALWAYS && node.onExecute) {
                        //wrap node.onExecute();
						node.doExecute();
                    }
                }
    ...
    LGraph.prototype.computeExecutionOrder = function(
        ...
        if (L.length != this._nodes.length && HeavyGraph.debug) {
    ...
    LGraph.prototype.arrange = function (margin, layout) {
        ...
        for (let i = 0; i < columns.length; ++i) {
            ...
            let max_size = 100;
            let y = margin + HeavyGraph.NODE_TITLE_HEIGHT;
            for (let j = 0; j < column.length; ++j) {
                const node = column[j];
                node.pos[0] = (layout == HeavyGraph.VERTICAL_LAYOUT) ? y : x;
                node.pos[1] = (layout == HeavyGraph.VERTICAL_LAYOUT) ? x : y;
                const max_size_index = (layout == HeavyGraph.VERTICAL_LAYOUT) ? 1 : 0;
                if (node.size[max_size_index] > max_size) {
                    max_size = node.size[max_size_index];
                }
                const node_size_index = (layout == HeavyGraph.VERTICAL_LAYOUT) ? 0 : 1;
                y += node.size[node_size_index] + margin + HeavyGraph.NODE_TITLE_HEIGHT;
            }
            x += max_size + margin;
        }
    ...
    LGraph.prototype.sendEventToAllNodes = function(eventname, params, mode) {
        ...
        for (var j = 0, l = nodes.length; j < l; ++j) {
            ...
            if (
                node.constructor === HeavyGraph.Subgraph &&
                eventname != "onExecute"
            ) {
                if (node.mode == mode) {
    ...
    LGraph.prototype.add = function(node, skip_compute_order) {
        ...
        if (node.id != -1 && this._nodes_by_id[node.id] != null) {
            console.warn(
                "HeavyGraph: there is already a node with this ID, changing it"
            );
            if (HeavyGraph.use_uuids) {
                node.id = HeavyGraph.uuidv4();
            }
            else {
        ...
        if (this._nodes.length >= HeavyGraph.MAX_NUMBER_OF_NODES) {
            throw "HeavyGraph: max number of nodes in a graph reached";
        }
    ...
    LGraph.prototype.remove = function(node) {
        if (node.constructor === HeavyGraph.LGraphGroup) {
    ...
    LGraph.prototype.getNodeOnPos = function(x, y, nodes_list, margin) {
        ...
        for (var i = nodes_list.length - 1; i >= 0; i--) {
            var n = nodes_list[i];
            var skip_title = n.constructor.title_mode == HeavyGraph.NO_TITLE;
    ...
    LGraph.prototype.checkNodeTypes = function() {
        ...
        for (var i = 0; i < this._nodes.length; i++) {
            var node = this._nodes[i];
            var ctor = HeavyGraph.registered_node_types[node.type];
            if (node.constructor == ctor) {
                continue;
            }
            console.log("node being replaced by newer version: " + node.type);
            var newnode = HeavyGraph.createNode(node.type);
            changes = true;
    ...
    LGraph.prototype.configure = function(data, keep_old) {
        ...
        if (nodes) {
            for (var i = 0, l = nodes.length; i < l; ++i) {
                ...
                var node = HeavyGraph.createNode(n_info.type, n_info.title);
                if (!node) {
                    if (HeavyGraph.debug) {
                        console.log(
        ...
        if (data.groups) {
            for (var i = 0; i < data.groups.length; ++i) {
                var group = new HeavyGraph.LGraphGroup();
	...
	supported callbacks:
		...
		+ onConnectionsChange : a connection changed (new one or removed) (HeavyGraph.INPUT or HeavyGraph.OUTPUT, slot, true if connected, link_info, input_info )
...
*/
    ...
    global.LGraphNode = HeavyGraph.LGraphNode = LGraphNode;

    LGraphNode.prototype._ctor = function(title) {
        this.title = title || "Unnamed";
        this.size = [HeavyGraph.NODE_WIDTH, 60];
        this.graph = null;
        ...
        for (var j in info) {
            ...
            if (info[j] == null) {
                ...
                if (this[j] && this[j].configure) {
                    ...
                    this[j] = HeavyGraph.cloneObject(info[j], this[j]);
                }
            } //value
            else {
                this[j] = info[j];
		...
		if (this.inputs) {
			for (var i = 0; i < this.inputs.length; ++i) {
				...
				if (this.onConnectionsChange)
					this.onConnectionsChange( HeavyGraph.INPUT, i, true, link_info, input ); //link_info has been created now, so its updated
		...
		if (this.outputs) {
			for (var i = 0; i < this.outputs.length; ++i) {
				...
				for (var j = 0; j < output.links.length; ++j) {
					...
					if (this.onConnectionsChange)
						this.onConnectionsChange( HeavyGraph.OUTPUT, i, true, link_info, output ); //link_info has been created now, so its updated
				}
    ...
    LGraphNode.prototype.serialize = function() {
        ...
        var o = {
            ...
            pos: this.pos,
            size: this.size,
            flags: HeavyGraph.cloneObject(this.flags),
			order: this.order,
            mode: this.mode
        };
        ...
        if (this.properties) {
            o.properties = HeavyGraph.cloneObject(this.properties);
        }
    ...
    LGraphNode.prototype.clone = function() {
        var node = HeavyGraph.createNode(this.type);
        if (!node) {
            return null;
        }

        //we clone it because serialize returns shared containers
        var data = HeavyGraph.cloneObject(this.serialize());
    ...
    //LGraphNode.prototype.deserialize = function(info) {} //this cannot be done from within, must be done in HeavyGraph
    ...
    LGraphNode.prototype.addOnTriggerInput = function(){
        ...
        if (trigS == -1){ //!trigS || 
            var input = this.addInput("onTrigger", HeavyGraph.EVENT, {optional: true, nameLocked: true});
    ...
    LGraphNode.prototype.addOnExecutedOutput = function(){
        ...
        if (trigS == -1){ //!trigS || 
            var output = this.addOutput("onExecuted", HeavyGraph.ACTION, {optional: true, nameLocked: true});
    ...
    LGraphNode.prototype.changeMode = function(modeTo){
        switch(modeTo){
            case HeavyGraph.ON_EVENT:
                // this.addOnExecutedOutput();
                break;
                
            case HeavyGraph.ON_TRIGGER:
                this.addOnTriggerInput();
                this.addOnExecutedOutput();
                break;
                
            case HeavyGraph.NEVER:
                break;
                
            case HeavyGraph.ALWAYS:
                break;
                
            case HeavyGraph.ON_REQUEST:
                break;
    ...
    LGraphNode.prototype.trigger = function(action, param, options) {
        ...
        if (this.graph)
            this.graph._last_trigger_time = HeavyGraph.getTime();

        for (var i = 0; i < this.outputs.length; ++i) {
            var output = this.outputs[i];
            if ( !output || output.type !== HeavyGraph.EVENT || (action && output.name != action) )
                continue;
    ...
    LGraphNode.prototype.triggerSlot = function(slot, param, link_id, options) {
        ...
        for (var k = 0; k < links.length; ++k) {
            ...
            link_info._last_time = HeavyGraph.getTime();
			...
			if (node.mode === HeavyGraph.ON_TRIGGER)
			{
    ...
    LGraphNode.prototype.addOutput = function(name, type, extra_info) {
        ...
        if (HeavyGraph.auto_load_slot_types) HeavyGraph.registerNodeAndSlotType(this,type,true);
    ...
    LGraphNode.prototype.addOutputs = function(array) {
        for (var i = 0; i < array.length; ++i) {
            ...
            if (HeavyGraph.auto_load_slot_types) HeavyGraph.registerNodeAndSlotType(this,info[1],true);
    ...
    LGraphNode.prototype.addInput = function(name, type, extra_info) {
        ...
        HeavyGraph.registerNodeAndSlotType(this,type);
    ...
    LGraphNode.prototype.addInputs = function(array) {
        for (var i = 0; i < array.length; ++i) {
            ...
            HeavyGraph.registerNodeAndSlotType(this,info[1]);
        }
    ...
    LGraphNode.prototype.computeSize = function(out) {
        ...
        var font_size = HeavyGraph.NODE_TEXT_SIZE; //although it should be graphcanvas.inner_text_font size
        ...
        size[0] = Math.max(size[0], HeavyGraph.NODE_WIDTH);
        if (this.widgets && this.widgets.length) {
            size[0] = Math.max(size[0], HeavyGraph.NODE_WIDTH * 1.5);
        }

        size[1] = (this.constructor.slot_start_y || 0) + rows * HeavyGraph.NODE_SLOT_HEIGHT;
        ...
        if (this.widgets && this.widgets.length) {
            for (var i = 0, l = this.widgets.length; i < l; ++i) {
                if (this.widgets[i].computeSize)
                    ...
                    widgets_height += HeavyGraph.NODE_WIDGET_HEIGHT + 4;
            }
            widgets_height += 8;
        }
    ...
    LGraphNode.prototype.inResizeCorner = function(canvasX, canvasY) {
        ...
        var outputs_offset = (this.constructor.slot_start_y || 0) + rows * HeavyGraph.NODE_SLOT_HEIGHT;
    ...
    */
    ...
    LGraphNode.prototype.addWidget = function( type, name, value, callback, options )
	{
        ...
        if (!callback && !w.options.callback && !w.options.property) {
            console.warn("HeavyGraph addWidget(...) without a callback or property assigned");
        }
        if (type == "combo" && !w.options.values) {
            throw "HeavyGraph addWidget('combo',...) requires to pass values in options: { values:['red','blue'] }";
        }
        this.widgets.push(w);
		this.setSize( this.computeSize() );
        return w;
    ...
    LGraphNode.prototype.getBounding = function(out, compute_outer) {
        ...
        out[1] = nodePos[1] - HeavyGraph.NODE_TITLE_HEIGHT - top_offset;
        out[2] = isCollapsed ?
            (this._collapsed_width || HeavyGraph.NODE_COLLAPSED_WIDTH) + right_offset :
            nodeSize[0] + right_offset;
        out[3] = isCollapsed ?
            HeavyGraph.NODE_TITLE_HEIGHT + bottom_offset :
            nodeSize[1] + HeavyGraph.NODE_TITLE_HEIGHT + bottom_offset;
    ...
    LGraphNode.prototype.isPointInside = function(x, y, margin, skip_title) {
        ...
        var margin_top = this.graph && this.graph.isLive() ? 0 : HeavyGraph.NODE_TITLE_HEIGHT;
        if (skip_title) {
            margin_top = 0;
        }
        if (this.flags && this.flags.collapsed) {
            //if ( distance([x,y], [this.pos[0] + this.size[0]*0.5, this.pos[1] + this.size[1]*0.5]) < HeavyGraph.NODE_COLLAPSED_RADIUS)
            if (
                isInsideRectangle(
                    x,
                    y,
                    this.pos[0] - margin,
                    this.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT - margin,
                    (this._collapsed_width || HeavyGraph.NODE_COLLAPSED_WIDTH) +
                        2 * margin,
                    HeavyGraph.NODE_TITLE_HEIGHT + 2 * margin
                )
            ) {
                return true;
            }
        ...
        for (var i = 0, l = aSlots.length; i < l; ++i) {
            ...
            for(var sI=0;sI<aSource.length;sI++){
                for(var dI=0;dI<aDest.length;dI++){
					if (aSource[sI]=="_event_") aSource[sI] = HeavyGraph.EVENT;
					if (aDest[sI]=="_event_") aDest[sI] = HeavyGraph.EVENT;
    ...
    LGraphNode.prototype.connectByType = function(slot, target_node, target_slotType, optsIn) {
        ...
        if (target_slot >= 0 && target_slot !== null){
            ...
            if (opts.createEventInCase && target_slotType == HeavyGraph.EVENT){
            ...
            if (opts.firstFreeIfOutputGeneralInCase && (target_slotType == 0 || target_slotType == "*" || target_slotType == "")){
                var target_slot = target_node.findInputSlotFree({typesNotAccepted: [HeavyGraph.EVENT] });
    ...
    LGraphNode.prototype.connectByTypeOutput = function(slot, source_node, source_slotType, optsIn) {
        ...
        if (source_slot >= 0 && source_slot !== null){
            ...
            if (opts.createEventInCase && source_slotType == HeavyGraph.EVENT){
                // WILL CREATE THE onExecuted OUT SLOT
				if (HeavyGraph.do_add_triggers_slots){
            ...
            if (opts.firstFreeIfInputGeneralInCase && (source_slotType == 0 || source_slotType == "*" || source_slotType == "")){
                var source_slot = source_node.findOutputSlotFree({typesNotAccepted: [HeavyGraph.EVENT] });
                if (source_slot >= 0){
                    return source_node.connect(source_slot, this, slot);
                }
            }
    ...
    LGraphNode.prototype.connect = function(slot, target_node, target_slot) {
        ...
        if (target_slot===false || target_slot===null || !HeavyGraph.isValidConnection(output.type, input.type))
		{
        ...
        if (output.links !== null && output.links.length){
            switch(output.type){
                case HeavyGraph.EVENT:
                    if (!HeavyGraph.allow_multi_output_for_events){
                        this.graph.beforeChange();
		...
		if(link_id != null)
		{
			...
			if (link_info) {
				...
				if (this.graph && this.graph.onNodeConnectionChange) {
					...
					this.graph.onNodeConnectionChange(HeavyGraph.INPUT, this, slot);
				}
			}
		} //link != null
    ...
    LGraphNode.prototype.getConnectionPos = function(
        ...
        var offset = HeavyGraph.NODE_SLOT_HEIGHT * 0.5;

        if (this.flags.collapsed) {
            var w = this._collapsed_width || HeavyGraph.NODE_COLLAPSED_WIDTH;
            if (this.horizontal) {
                out[0] = this.pos[0] + w * 0.5;
                if (is_input) {
                    out[1] = this.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT;
                } else {
                    out[1] = this.pos[1];
                }
            } else {
                ...
                out[1] = this.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT * 0.5;
            }
            return out;
        }
        ...
        if (is_input && slot_number == -1) {
            out[0] = this.pos[0] + HeavyGraph.NODE_TITLE_HEIGHT * 0.5;
            out[1] = this.pos[1] + HeavyGraph.NODE_TITLE_HEIGHT * 0.5;
            return out;
        }
        ...
        out[1] =
            this.pos[1] +
            (slot_number + 0.7) * HeavyGraph.NODE_SLOT_HEIGHT +
    ...
    LGraphNode.prototype.alignToGrid = function() {
        this.pos[0] =
            HeavyGraph.CANVAS_GRID_SIZE *
            Math.round(this.pos[0] / HeavyGraph.CANVAS_GRID_SIZE);
        this.pos[1] =
            HeavyGraph.CANVAS_GRID_SIZE *
            Math.round(this.pos[1] / HeavyGraph.CANVAS_GRID_SIZE);
    };
    ...
    LGraphNode.prototype.loadImage = function(url) {
        var img = new Image();
        img.src = HeavyGraph.node_images_path + url;
        img.ready = false;
...
*/
    ...
    global.LGraphGroup = HeavyGraph.LGraphGroup = LGraphGroup;
    ...
    HeavyGraph.DragAndScale = DragAndScale;
    ...
    DragAndScale.prototype.bindEvents = function(element) {
		...
		HeavyGraph.pointerListenerAdd(element,"down", this._binded_mouse_callback);
		HeavyGraph.pointerListenerAdd(element,"move", this._binded_mouse_callback);
		HeavyGraph.pointerListenerAdd(element,"up", this._binded_mouse_callback);
    ...
    DragAndScale.prototype.onMouse = function(e) {
        ...
        if (e.type == HeavyGraph.pointerevents_method+"down" && is_inside) {
            this.dragging = true;
			HeavyGraph.pointerListenerRemove(canvas,"move",this._binded_mouse_callback);
			HeavyGraph.pointerListenerAdd(document,"move",this._binded_mouse_callback);
			HeavyGraph.pointerListenerAdd(document,"up",this._binded_mouse_callback);
        } else if (e.type == HeavyGraph.pointerevents_method+"move") {
            if (!ignore) {
        ...
        } else if (e.type == HeavyGraph.pointerevents_method+"up") {
            this.dragging = false;
			HeavyGraph.pointerListenerRemove(document,"move",this._binded_mouse_callback);
			HeavyGraph.pointerListenerRemove(document,"up",this._binded_mouse_callback);
			HeavyGraph.pointerListenerAdd(canvas,"move",this._binded_mouse_callback);
        } else if ( is_inside &&
            (e.type == "mousewheel" ||
            e.type == "wheel" ||
    ...
    function LGraphCanvas(canvas, graph, options) {
        ...
        this.title_text_font = "" + HeavyGraph.NODE_TEXT_SIZE + "px Arial";
        this.inner_text_font =
            "normal " + HeavyGraph.NODE_SUBTEXT_SIZE + "px Arial";
        this.node_title_color = HeavyGraph.NODE_TITLE_COLOR;
        this.default_link_color = HeavyGraph.LINK_COLOR;
        ...
        this.links_render_mode = HeavyGraph.SPLINE_LINK;
    ...
    global.LGraphCanvas = HeavyGraph.LGraphCanvas = LGraphCanvas;
    ...
    LGraphCanvas.prototype.setCanvas = function(canvas, skip_events) {
        ...
        if (canvas) {
            if (canvas.constructor === String) {
                ...
                if (!canvas) {
                    throw "Error creating HeavyGraph canvas: Canvas not found";
                }
            }
        }
    ...
    LGraphCanvas.prototype.bindEvents = function() {
		...
		HeavyGraph.pointerListenerAdd(canvas,"down", this._mousedown_callback, true); //down do not need to store the binded
        canvas.addEventListener("mousewheel", this._mousewheel_callback, false);

        HeavyGraph.pointerListenerAdd(canvas,"up", this._mouseup_callback, true); // CHECK: ??? binded or not
		HeavyGraph.pointerListenerAdd(canvas,"move", this._mousemove_callback);
    ...
    LGraphCanvas.prototype.unbindEvents = function() {
		...
		HeavyGraph.pointerListenerRemove(this.canvas,"move", this._mousedown_callback);
        HeavyGraph.pointerListenerRemove(this.canvas,"up", this._mousedown_callback);
        HeavyGraph.pointerListenerRemove(this.canvas,"down", this._mousedown_callback);
	...
	*/
    ...
    LGraphCanvas.prototype.processMouseDown = function(e) {
		...
		{
			HeavyGraph.pointerListenerRemove(this.canvas,"move", this._mousemove_callback);
			HeavyGraph.pointerListenerAdd(ref_window.document,"move", this._mousemove_callback,true); //catch for the entire window
			HeavyGraph.pointerListenerAdd(ref_window.document,"up", this._mouseup_callback,true);
		}
        ...
        HeavyGraph.closeAllContextMenus(ref_window);
        ...
        if (e.which == 1 && !this.pointer_is_double)
		{
            ...
            if (HeavyGraph.alt_drag_do_clone_nodes && e.altKey && node && this.allow_interaction && !skip_action && !this.read_only)
            {
            ...
            if (node && (this.allow_interaction || node.flags.allow_interaction) && !skip_action && !this.read_only) {
                        ...
                        if (node.outputs) {
                            for ( var i = 0, l = node.outputs.length; i < l; ++i ) {
                                ...
                                if (
                                    ...
                                    if (HeavyGraph.shift_click_do_break_link_from){
                                        if (e.shiftKey) {
                                            node.disconnectOutput(i);
                                        }
                                    }
                            ...
                            for ( var i = 0, l = node.inputs.length; i < l; ++i ) {
                                ...
                                if (
                                    ...
                                    if (input.link !== null) {
                                        ...
                                        if (HeavyGraph.click_do_break_link_to){
                                            node.disconnectInput(i);
                                            this.dirty_bgcanvas = true;
                                            skip_action = true;
                                        }else{
                ...
                if (!skip_action) {
                    ...
                    if ( node.onMouseDown && node.onMouseDown( e, pos, this ) ) {
						...
						{
							if ( !node.flags.collapsed && pos[0] > node.size[0] - HeavyGraph.NODE_TITLE_HEIGHT && pos[1] < 0 ) {
								var that = this;
								setTimeout(function() {
			...
			if (HeavyGraph.middle_click_slot_add_default_node){
        ...
        this.last_mouseclick = HeavyGraph.getTime();
	...
	*/
                    ...
                    if (this.connecting_output){
                        ...
                        if (this.isOverNodeBox(node, e.canvasX, e.canvasY)) {
                            ...
                            if (slot != -1 && node.inputs[slot]) {
                                ...
                                if ( HeavyGraph.isValidConnection( this.connecting_output.type, slot_type ) ) {
                        ...
                        if (this.isOverNodeBox(node, e.canvasX, e.canvasY)) {
                            ...
                            if (slot != -1 && node.outputs[slot]) {
                                ...
                                if ( HeavyGraph.isValidConnection( this.connecting_input.type, slot_type ) ) {
    ...
    LGraphCanvas.prototype.processMouseUp = function(e) {
		...
		if(!this.options.skip_events)
		{
			...
			HeavyGraph.pointerListenerRemove(document,"move", this._mousemove_callback,true);
			HeavyGraph.pointerListenerAdd(this.canvas,"move", this._mousemove_callback,true);
			HeavyGraph.pointerListenerRemove(document,"up", this._mouseup_callback,true);
		}
        ...
        if (e.which == 1) {
            ...
            if (this.dragging_rectangle) {
                ...
                if (node) {
                	...
                	if (HeavyGraph.release_link_on_empty_shows_menu){
                ...
                if (
                    node &&
                    e.click_time < 300 &&
                    isInsideRectangle( e.canvasX, e.canvasY, node.pos[0], node.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT, HeavyGraph.NODE_TITLE_HEIGHT, HeavyGraph.NODE_TITLE_HEIGHT )
                ) {
                    node.collapse();
                }
    ...
    LGraphCanvas.prototype.isOverNodeBox = function(node, canvasx, canvasy) {
        var title_height = HeavyGraph.NODE_TITLE_HEIGHT;
        if (
            isInsideRectangle(
                canvasx,
                canvasy,
    ...
    LGraphCanvas.prototype.pasteFromClipboard = function(isConnectUnselected = false) {
        ...
        if (!HeavyGraph.ctrl_shift_v_paste_connect_unselected_outputs && isConnectUnselected) {
            return;
        }
        ...
        for (var i = 0; i < clipboard_info.nodes.length; ++i) {
            ...
            var node = HeavyGraph.createNode(node_data.type);
            if (node) {
        ...
        for (var i = 0; i < clipboard_info.links.length; ++i) {
            ...
            } else if (HeavyGraph.ctrl_shift_v_paste_connect_unselected_outputs && isConnectUnselected) {
    ...
    LGraphCanvas.prototype.checkDropItem = function(e) {
        if (e.dataTransfer.files.length) {
            ...
            var nodetype = HeavyGraph.node_types_by_file_extension[ext];
            if (nodetype) {
				this.graph.beforeChange();
                var node = HeavyGraph.createNode(nodetype.type);
    ...
    LGraphCanvas.prototype.deleteSelectedNodes = function() {
        ...
        for (var i in this.selected_nodes) {
			...
			if(node.inputs && node.inputs.length && node.outputs && node.outputs.length && HeavyGraph.isValidConnection( node.inputs[0].type, node.outputs[0].type ) && node.inputs[0].link && node.outputs[0].links && node.outputs[0].links.length ) 
			{
	...
	*/
    ...
    LGraphCanvas.prototype.drawFrontCanvas = function() {
        ...
        if (this.graph) {
            ...
            if (this.connecting_pos != null) {
				...
				{
					if (this.connecting_output)
						connDir = this.connecting_node.horizontal ? HeavyGraph.DOWN : HeavyGraph.RIGHT;
					else
						connDir = this.connecting_node.horizontal ? HeavyGraph.UP : HeavyGraph.LEFT;
				}
                var connShape = connInOrOut.shape;
                
                switch (connType) {
                    case HeavyGraph.EVENT:
                        link_color = HeavyGraph.EVENT_LINK_COLOR;
                        break;
                    default:
                        link_color = HeavyGraph.CONNECTING_LINK_COLOR;
                }
                ...
                if (
                    connType === HeavyGraph.EVENT ||
                    connShape === HeavyGraph.BOX_SHAPE
                ) {
                    ctx.rect(
                ...
                } else if (connShape === HeavyGraph.ARROW_SHAPE) {
                ...
                if (this._highlight_input) {
                    ...
                    if (shape === HeavyGraph.ARROW_SHAPE) {
    ...
    LGraphCanvas.prototype.drawSubgraphPanelLeft = function (subgraph, subnode, ctx) {
        ...
        var w = 200;
        var h = Math.floor(HeavyGraph.NODE_SLOT_HEIGHT * 1.6);
        ...
        if (subnode.inputs)
            for (var i = 0; i < subnode.inputs.length; ++i) {
                ...
                if (this.drawButton(20, y + 2, w - 20, h - 2)) {
                    ...
                    this.graph.beforeChange();
                    var newnode = HeavyGraph.createNode(type);
                    if (newnode) {
	...
	{
		var ctx = this.ctx;
		bgcolor = bgcolor || HeavyGraph.NODE_DEFAULT_COLOR;
		hovercolor = hovercolor || "#555";
		textcolor = textcolor || HeavyGraph.NODE_TEXT_COLOR;
		var pos = this.ds.convertOffsetToCanvas(this.graph_mouse);
		var hover = HeavyGraph.isInsideRectangle( pos[0], pos[1], x,y,w,h );
		...
		var clicked = pos && HeavyGraph.isInsideRectangle( pos[0], pos[1], x,y,w,h );
    ...
    LGraphCanvas.prototype.drawNode = function(node, ctx) {
        ...
        var color = node.color || node.constructor.color || HeavyGraph.NODE_DEFAULT_COLOR;
        var bgcolor = node.bgcolor || node.constructor.bgcolor || HeavyGraph.NODE_DEFAULT_BGCOLOR;
        ...
        if (this.render_shadows && !low_quality) {
            ctx.shadowColor = HeavyGraph.DEFAULT_SHADOW_COLOR;
        ...
        var shape = node._shape || HeavyGraph.BOX_SHAPE;
        var size = temp_vec2;
            ...
            if (title != null) {
                node._collapsed_width = Math.min(
                    ...
                    ctx.measureText(title).width +
                        HeavyGraph.NODE_TITLE_HEIGHT * 2
                ); //HeavyGraph.NODE_COLLAPSED_WIDTH;
        ...
        if (node.clip_area) {
            ...
            ctx.save();
            ctx.beginPath();
            if (shape == HeavyGraph.BOX_SHAPE) {
                ctx.rect(0, 0, size[0], size[1]);
            } else if (shape == HeavyGraph.ROUND_SHAPE) {
                ctx.roundRect(0, 0, size[0], size[1], [10]);
            } else if (shape == HeavyGraph.CIRCLE_SHAPE) {
                ctx.arc(
                    size[0] * 0.5,
                    size[1] * 0.5,
                    size[0] * 0.5,
        ...
        if (!node.flags.collapsed) {
            ...
            if (node.inputs) {
                for (var i = 0; i < node.inputs.length; i++) {
                    ...
                    if ( this.connecting_output && !HeavyGraph.isValidConnection( slot.type , out_slot.type) ) {
                    ...
                    pos[0] -= node.pos[0];
                    pos[1] -= node.pos[1];
                    if (max_y < pos[1] + HeavyGraph.NODE_SLOT_HEIGHT * 0.5) {
                        max_y = pos[1] + HeavyGraph.NODE_SLOT_HEIGHT * 0.5;
                    }

                    ctx.beginPath();

					if (slot_type == "array"){
                        slot_shape = HeavyGraph.GRID_SHAPE; // place in addInput? addOutput instead?
                    }
                    
                    var doStroke = true;
                    
                    if (
                        slot.type === HeavyGraph.EVENT ||
                        slot.shape === HeavyGraph.BOX_SHAPE
                    ) {
                        if (horizontal) {
                            ctx.rect(
                                pos[0] - 5 + 0.5,
                    ...
                    } else if (slot_shape === HeavyGraph.ARROW_SHAPE) {
                        ctx.moveTo(pos[0] + 8, pos[1] + 0.5);
                        ctx.lineTo(pos[0] - 4, pos[1] + 6 + 0.5);
                        ctx.lineTo(pos[0] - 4, pos[1] - 6 + 0.5);
                        ctx.closePath();
                    } else if (slot_shape === HeavyGraph.GRID_SHAPE) {
                    ...
                    if (render_text) {
                        ...
                        if (text) {
                            ctx.fillStyle = HeavyGraph.NODE_TEXT_COLOR;
                            if (horizontal || slot.dir == HeavyGraph.UP) {
            ...
            if (node.outputs) {
                for (var i = 0; i < node.outputs.length; i++) {
                    ...
                    //change opacity of incompatible slots when dragging a connection
                    if (this.connecting_input && !HeavyGraph.isValidConnection( slot_type , in_slot.type) ) {
                        ctx.globalAlpha = 0.4 * editor_alpha;
                    }
					...
					if (slot_type == "array"){
                        slot_shape = HeavyGraph.GRID_SHAPE;
                    }
                    
                    var doStroke = true;
                    
                    if (
                        slot_type === HeavyGraph.EVENT ||
                        slot_shape === HeavyGraph.BOX_SHAPE
                    ) {
                        if (horizontal) {
                            ctx.rect(
                                pos[0] - 5 + 0.5,
                    ...
                    if (render_text) {
                        ...
                        if (text) {
                            ctx.fillStyle = HeavyGraph.NODE_TEXT_COLOR;
                            if (horizontal || slot.dir == HeavyGraph.DOWN) {
            ...
            if (input_slot) {
                var x = 0;
                var y = HeavyGraph.NODE_TITLE_HEIGHT * -0.5; //center
                if (horizontal) {
                    x = node._collapsed_width * 0.5;
                    y = -HeavyGraph.NODE_TITLE_HEIGHT;
                }
                ctx.fillStyle = "#686";
                ctx.beginPath();
                if (
                ...
                } else if (slot.shape === HeavyGraph.ARROW_SHAPE) {
                    ctx.moveTo(x + 8, y);
                    ctx.lineTo(x + -4, y - 4);
                    ctx.lineTo(x + -4, y + 4);
                    ctx.closePath();
    ...
    LGraphCanvas.prototype.drawNodeShape = function(
        ...
        var shape =
            node._shape || node.constructor.shape || HeavyGraph.ROUND_SHAPE;

        var title_mode = node.constructor.title_mode;

        var render_title = true;
        if (title_mode == HeavyGraph.TRANSPARENT_TITLE || title_mode == HeavyGraph.NO_TITLE) {
            render_title = false;
        } else if (title_mode == HeavyGraph.AUTOHIDE_TITLE && mouse_over) {
            render_title = true;
        }
        ...
        {
            ctx.beginPath();
            if (shape == HeavyGraph.BOX_SHAPE || low_quality) {
                ctx.fillRect(area[0], area[1], area[2], area[3]);
            } else if (
                shape == HeavyGraph.ROUND_SHAPE ||
                shape == HeavyGraph.CARD_SHAPE
            ) {
                ctx.roundRect(
                    area[0],
                    area[1],
                    area[2],
                    area[3],
                    shape == HeavyGraph.CARD_SHAPE ? [this.round_radius,this.round_radius,0,0] : [this.round_radius] 
                );
            } else if (shape == HeavyGraph.CIRCLE_SHAPE) {
                ctx.arc(
                    size[0] * 0.5,
        ...
        if (render_title || title_mode == HeavyGraph.TRANSPARENT_TITLE) {
            //title bar
            if (node.onDrawTitleBar) {
                node.onDrawTitleBar( ctx, title_height, size, this.ds.scale, fgcolor );
            } else if (
                title_mode != HeavyGraph.TRANSPARENT_TITLE &&
                ...
                } else if (  shape == HeavyGraph.ROUND_SHAPE || shape == HeavyGraph.CARD_SHAPE ) {
                    ctx.roundRect(
                        0,
                        -title_height,
                        size[0] + 1,
            ...
            var colState = false;
            if (HeavyGraph.node_box_coloured_by_mode){
                if(HeavyGraph.NODE_MODES_COLORS[node.mode]){
                    colState = HeavyGraph.NODE_MODES_COLORS[node.mode];
                }
            }
            if (HeavyGraph.node_box_coloured_when_on){
            ...
            if (node.onDrawTitleBox) {
                ...
                ctx.fillStyle = node.boxcolor || colState || HeavyGraph.NODE_DEFAULT_BOXCOLOR;
				if(low_quality)
            ...
            if (!low_quality) {
                ...
                if (title) {
                    if (selected) {
                        ctx.fillStyle = HeavyGraph.NODE_SELECTED_TITLE_COLOR;
                    } else {
                        ctx.fillStyle =
                    ...
                    if (node.flags.collapsed) {
                        ...
                        ctx.fillText(
                            ...
                            HeavyGraph.NODE_TITLE_TEXT_Y - title_height
                        );
                        ctx.textAlign = "left";
                    } else {
                        ctx.textAlign = "left";
			...
			if (!node.flags.collapsed && node.subgraph && !node.skip_subgraph_button) {
				var w = HeavyGraph.NODE_TITLE_HEIGHT;
				var x = node.size[0] - w;
				var over = HeavyGraph.isInsideRectangle( this.graph_mouse[0] - node.pos[0], this.graph_mouse[1] - node.pos[1], x+2, -w+2, w-4, w-4 );
        ...
        if (selected) {
            ...
            if (title_mode == HeavyGraph.TRANSPARENT_TITLE) {
                area[1] -= title_height;
                area[3] += title_height;
            }
            ctx.lineWidth = 1;
            ...
            if (shape == HeavyGraph.BOX_SHAPE) {
                ...
                shape == HeavyGraph.ROUND_SHAPE ||
                (shape == HeavyGraph.CARD_SHAPE && node.flags.collapsed)
            ) {
                ctx.roundRect(
                    -6 + area[0],
                    -6 + area[1],
            ...
            } else if (shape == HeavyGraph.CARD_SHAPE) {
                ctx.roundRect(
                    -6 + area[0],
                    -6 + area[1],
                    12 + area[2],
            ...
            ctx.strokeStyle = HeavyGraph.NODE_BOX_OUTLINE_COLOR;
            ctx.stroke();
            ctx.strokeStyle = fgcolor;
            ctx.globalAlpha = 1;
        }
    ...
    LGraphCanvas.prototype.drawConnections = function(ctx) {
        ...
        for (var n = 0, l = nodes.length; n < l; ++n) {
            ...
            for (var i = 0; i < node.inputs.length; ++i) {
                ...
                var start_dir =
                    start_slot.dir ||
                    (start_node.horizontal ? HeavyGraph.DOWN : HeavyGraph.RIGHT);
                var end_dir =
                    end_slot.dir ||
                    (node.horizontal ? HeavyGraph.UP : HeavyGraph.LEFT);
    ...
    LGraphCanvas.prototype.renderLink = function(
        ...
        start_dir = start_dir || HeavyGraph.RIGHT;
        end_dir = end_dir || HeavyGraph.LEFT;
        ...
        for (var i = 0; i < num_sublines; i += 1) {
            ...
            if (this.links_render_mode == HeavyGraph.SPLINE_LINK) {
                ...
                var end_offset_y = 0;
                switch (start_dir) {
                    case HeavyGraph.LEFT:
                        start_offset_x = dist * -0.25;
                        break;
                    case HeavyGraph.RIGHT:
                        start_offset_x = dist * 0.25;
                        break;
                    case HeavyGraph.UP:
                        start_offset_y = dist * -0.25;
                        break;
                    case HeavyGraph.DOWN:
                        start_offset_y = dist * 0.25;
                        break;
                }
                switch (end_dir) {
            ...
            } else if (this.links_render_mode == HeavyGraph.LINEAR_LINK) {
                ctx.moveTo(a[0], a[1] + offsety);
                var start_offset_x = 0;
                var start_offset_y = 0;
                var end_offset_x = 0;
            ...
            } else if (this.links_render_mode == HeavyGraph.STRAIGHT_LINK) {
                ctx.moveTo(a[0], a[1]);
                var start_x = a[0];
                var start_y = a[1];
                var end_x = b[0];
                var end_y = b[1];
                if (start_dir == HeavyGraph.RIGHT) {
                    start_x += 10;
                } else {
                    start_y += 10;
                }
                if (end_dir == HeavyGraph.LEFT) {
                    end_x -= 10;
                } else {
                    end_y -= 10;
                }
        ...
        if (flow) {
            ctx.fillStyle = color;
            for (var i = 0; i < 5; ++i) {
                var f = (HeavyGraph.getTime() * 0.001 + i * 0.2) % 1;
    ...
    LGraphCanvas.prototype.drawExecutionOrder = function(ctx) {
        ...
        for (var i = 0; i < visible_nodes.length; ++i) {
            ...
            ctx.fillStyle = "black";
            ctx.fillRect(
                node.pos[0] - HeavyGraph.NODE_TITLE_HEIGHT,
                node.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT,
                HeavyGraph.NODE_TITLE_HEIGHT,
                HeavyGraph.NODE_TITLE_HEIGHT
            );
            if (node.order == 0) {
                ctx.strokeRect(
                    node.pos[0] - HeavyGraph.NODE_TITLE_HEIGHT + 0.5,
                    node.pos[1] - HeavyGraph.NODE_TITLE_HEIGHT + 0.5,
                    HeavyGraph.NODE_TITLE_HEIGHT,
                    HeavyGraph.NODE_TITLE_HEIGHT
                );
            }
    ...
    LGraphCanvas.prototype.drawNodeWidgets = function(
        ...
        posY += 2;
        var H = HeavyGraph.NODE_WIDGET_HEIGHT;
        var show_text = this.ds.scale > 0.5;
        ctx.save();
        ctx.globalAlpha = this.editor_alpha;
        var outline_color = HeavyGraph.WIDGET_OUTLINE_COLOR;
        var background_color = HeavyGraph.WIDGET_BGCOLOR;
        var text_color = HeavyGraph.WIDGET_TEXT_COLOR;
		var secondary_text_color = HeavyGraph.WIDGET_SECONDARY_TEXT_COLOR;
        var margin = 15;
        ...
        for (var i = 0; i < widgets.length; ++i) {
            ...
            switch (w.type) {
                ...
                case "combo":
                    ...
                    ctx.beginPath();
					if(show_text)
	                    ctx.roundRect(margin, y, widget_width - margin * 2, H, [H * 0.05] );
					else
	                    ctx.rect(margin, y, widget_width - margin * 2, H );
                    ctx.fill();
                    if (show_text) {
    ...
    LGraphCanvas.prototype.processNodeWidgets = function(
        ...
        for (var i = 0; i < node.widgets.length; ++i) {
			...
			if(!w || w.disabled)
				continue;
			var widget_height = w.computeSize ? w.computeSize(width)[1] : HeavyGraph.NODE_WIDGET_HEIGHT;
			var widget_width = w.width || width;
			//outside
			if ( w != active_widget && 
			...
			switch (w.type) {
				case "button":
					if (event.type === HeavyGraph.pointerevents_method+"down") {
                        if (w.callback) {
                            setTimeout(function() {
				...
				case "combo":
					...
					if (allow_scroll && event.type == HeavyGraph.pointerevents_method+"move" && w.type == "number") {
                        if(event.deltaX)
					...
					} else if (event.type == HeavyGraph.pointerevents_method+"down") {
						var values = w.options.values;
						...
						if (w.type == "number") {
							...
							var menu = new HeavyGraph.ContextMenu(text_values, {
					...
					else if(event.type == HeavyGraph.pointerevents_method+"up" && w.type == "number")
					{
						var delta = x < 40 ? -1 : x > widget_width - 40 ? 1 : 0;
    ...
    LGraphCanvas.prototype.drawGroups = function(canvas, ctx) {
        ...
        for (var i = 0; i < groups.length; ++i) {
            ...
            var font_size =
                group.font_size || HeavyGraph.DEFAULT_GROUP_FONT_SIZE;
    ...
    LGraphCanvas.onNodeAlign = function(value, options, event, prev_menu, node) {
        new HeavyGraph.ContextMenu(["Top", "Bottom", "Left", "Right"], {
            event: event,
    ...
    LGraphCanvas.onMenuAdd = function (node, options, e, prev_menu, callback) {
        ...
        function inner_onMenuAdded(base_category ,prev_menu){
            ...
            var categories  = HeavyGraph.getNodeTypesCategories(canvas.filter || graph.filter).filter(function(category){return category.startsWith(base_category)});
            var entries = [];
            ...
            var nodes = HeavyGraph.getNodeTypesInCategory(base_category.slice(0, -1), canvas.filter || graph.filter );
            nodes.map(function(node){
                ...
                var entry = { value: node.type, content: node.title, has_submenu: false , callback : function(value, event, mouseEvent, contextMenu){
                        ...
                        var node = HeavyGraph.createNode(value.value);
                        if (node) {
            ...
            new HeavyGraph.ContextMenu( entries, { event: e, parentMenu: prev_menu }, ref_window );
    ...
    LGraphCanvas.showMenuNodeOptionalInputs = function(
        ...
        var menu = new HeavyGraph.ContextMenu(
            entries,
            {
                event: e,
                callback: inner_clicked,
    ...
    LGraphCanvas.showMenuNodeOptionalOutputs = function(
        ...
        if (HeavyGraph.do_add_triggers_slots){ //canvas.allow_addOutSlot_onExecuted
            if (node.findOutputSlot("onExecuted") == -1){
                entries.push({content: "On Executed", value: ["onExecuted", HeavyGraph.EVENT, {nameLocked: true}], className: "event"}); //, opts: {}
            }
        }
        ...
        function inner_clicked(v, e, prev) {
            ...
            if (
                ...
                new HeavyGraph.ContextMenu(entries, {
                    event: e,
                    callback: inner_clicked,
                    parentMenu: prev_menu,
                    node: node
    ...
    LGraphCanvas.prototype.showLinkMenu = function(link, e) {
        ...
        var menu = new HeavyGraph.ContextMenu(options, {
            event: e,
 	...
 	LGraphCanvas.prototype.createDefaultNodeForSlot = function(optPass) { // addNodeMenu for connection
		...
		var fromSlotType = slotX.type==HeavyGraph.EVENT?"_event_":slotX.type;
		var slotTypesDefault = isFrom ? HeavyGraph.slot_types_default_out : HeavyGraph.slot_types_default_in;
		if(slotTypesDefault && slotTypesDefault[fromSlotType]){
			...
			if (nodeNewType) {
				...
				var newNode = HeavyGraph.createNode(nodeNewType);
				if(newNode){
    ...
    LGraphCanvas.onShowPropertyEditor = function(item, options, e, menu, node) {
        ...
        dialog.addEventListener("mouseleave", function(e) {
            if(HeavyGraph.dialog_close_on_mouse_leave)
                if (!dialog.is_modified && HeavyGraph.dialog_close_on_mouse_leave)
                    dialogCloseTimer = setTimeout(dialog.close, HeavyGraph.dialog_close_on_mouse_leave_delay); //dialog.close();
        });
    ...
    LGraphCanvas.prototype.prompt = function(title, value, callback, event, multiline) {
        ...
        HeavyGraph.pointerListenerAdd(dialog,"leave", function(e) {
            if (prevent_timeout)
                return;
            if(HeavyGraph.dialog_close_on_mouse_leave)
                if (!dialog.is_modified && HeavyGraph.dialog_close_on_mouse_leave)
        ...
        HeavyGraph.pointerListenerAdd(dialog,"enter", function(e) {
            if(HeavyGraph.dialog_close_on_mouse_leave)
                if(dialogCloseTimer) clearTimeout(dialogCloseTimer);
        });
    ...
    LGraphCanvas.prototype.showSearchBox = function(event, options) {
        ...
        var def_options = { slot_from: null
                        ,node_from: null
                        ,node_to: null
                        ,do_type_filter: HeavyGraph.search_filter_enabled // TODO check for registered_slot_[in/out]_types not empty // this will be checked for functionality enabled : filter on slot type, in and out
                        ,type_filter_in: false                          // these are default: pass to set initially set values
                        ,type_filter_out: false
                        ,show_general_if_none_on_typefilter: true
                        ,show_general_after_typefiltered: true
                        ,hide_on_mouse_leave: HeavyGraph.search_hide_on_mouse_leave
                        ,show_all_if_empty: true
                        ,show_all_on_open: HeavyGraph.search_show_all_on_open
                    };
        ...
        var dialog = document.createElement("div");
        dialog.className = "heavygraph litesearchbox graphdialog rounded";
        ...
        if (options.do_type_filter){
            if (selIn){
                var aSlots = HeavyGraph.slot_types_in;
                var nSlots = aSlots.length; // this for object :: Object.keys(aSlots).length;
                
                if (options.type_filter_in == HeavyGraph.EVENT || options.type_filter_in == HeavyGraph.ACTION)
            ...
            if (selOut){
                var aSlots = HeavyGraph.slot_types_out;
                var nSlots = aSlots.length; // this for object :: Object.keys(aSlots).length; 
                
                if (options.type_filter_out == HeavyGraph.EVENT || options.type_filter_out == HeavyGraph.ACTION)
		...
		*/
        ...
        function select(name) {
            if (name) {
                if (that.onSearchBoxSelection) {
                    ...
                    var extra = HeavyGraph.searchbox_extras[name.toLowerCase()];
                    if (extra) {
                        name = extra.type;
                    }

					graphcanvas.graph.beforeChange();
                    var node = HeavyGraph.createNode(name);
                    if (node) {
        ...
        function refreshHelper() {
            ...
            if (that.onSearchBox) {
                ...
                //extras
                for (var i in HeavyGraph.searchbox_extras) {
                    var extra = HeavyGraph.searchbox_extras[i];
                    if ((!options.show_all_if_empty || str) && extra.desc.toLowerCase().indexOf(str) === -1) {
                        continue;
                    }
					var ctor = HeavyGraph.registered_node_types[ extra.type ];
                ...
                if (Array.prototype.filter) { //filter supported
                    var keys = Object.keys( HeavyGraph.registered_node_types ); //types
                    var filtered = keys.filter( inner_test_filter );
                } else {
					filtered = [];
                    for (var i in HeavyGraph.registered_node_types) {
				...
				function inner_test_filter( type, optsIn )
				{
                    ...
                    var opts = Object.assign(optsDef,optsIn);
					var ctor = HeavyGraph.registered_node_types[ type ];
                    ...
                    if(options.do_type_filter && !opts.skipFilter){
						...
						//if (sV.toLowerCase() == "_event_") sV = HeavyGraph.EVENT; // -1
                        
                        if(sIn && sV){
                            //console.log("will check filter against "+sV);
                            if (HeavyGraph.registered_slot_in_types[sV] && HeavyGraph.registered_slot_in_types[sV].nodes){ // type is stored
                                //console.debug("check "+sType+" in "+HeavyGraph.registered_slot_in_types[sV].nodes);
                                var doesInc = HeavyGraph.registered_slot_in_types[sV].nodes.includes(sType);
                                if (doesInc!==false){
                                    //console.log(sType+" HAS "+sV);
                                }else{
                                    /*console.debug(HeavyGraph.registered_slot_in_types[sV]);
                        ...
                        if(sOut && sV){
                            ...
                            if (HeavyGraph.registered_slot_out_types[sV] && HeavyGraph.registered_slot_out_types[sV].nodes){ // type is stored
                                //console.debug("check "+sType+" in "+HeavyGraph.registered_slot_out_types[sV].nodes);
                                var doesInc = HeavyGraph.registered_slot_out_types[sV].nodes.includes(sType);
                                if (doesInc!==false){
                                    //console.log(sType+" HAS "+sV);
                                }else{
                                    /*console.debug(HeavyGraph.registered_slot_out_types[sV]);
                                    console.log(+" DONT includes "+type);*/
                                    return false;
                                }
                            }
            ...
            function addResult(type, className) {
                ...
                help.className = "heavygraph lite-search-item";
                if (className) {
    ...
    LGraphCanvas.prototype.createDialog = function(html, options) {
        ...
        dialog.addEventListener("mouseleave", function(e) {
            if (prevent_timeout)
                return;
            if(options.closeOnLeave || HeavyGraph.dialog_close_on_mouse_leave)
                if (!dialog.is_modified && HeavyGraph.dialog_close_on_mouse_leave)
                    dialogCloseTimer = setTimeout(dialog.close, HeavyGraph.dialog_close_on_mouse_leave_delay); //dialog.close();
        });
        dialog.addEventListener("mouseenter", function(e) {
	...
	LGraphCanvas.prototype.createPanel = function(title, options) {
		...
		root.className = "heavygraph dialog";
		...
		root.addWidget = function( type, name, value, options, callback )
		{
			...
			else if (type == "string" || type == "number")
			{
				...
				value_element.addEventListener("click", function(event){ 
					...
					var propname = this.parentNode.dataset["property"];
					var elem_that = this;
					var menu = new HeavyGraph.ContextMenu(values,{
							event: event,
							className: "dark",
							callback: inner_clicked
						},
    ...
    LGraphCanvas.prototype.showShowGraphOptionsPanel = function(refOpts, obEv, refMenu, refMenu2){
        ...
        function inner_refresh(){
            ...
            var aProps = HeavyGraph.availableCanvasOptions;
            aProps.sort();
            ...
            panel.addWidget( "combo", "Render mode", HeavyGraph.LINK_RENDER_MODES[graphcanvas.links_render_mode], {key: "links_render_mode", values: HeavyGraph.LINK_RENDER_MODES}, fUpdate);
    ...
    LGraphCanvas.prototype.showShowNodePanel = function( node )
	{
		...
		function inner_refresh()
		{
            ...
            var fUpdate = function(name,value){
                            ...
                            switch(name){
                                case "Title":
                                    ...
                                    break;
                                case "Mode":
                                    var kV = Object.values(HeavyGraph.NODE_MODES).indexOf(value);
                                    if (kV>=0 && HeavyGraph.NODE_MODES[kV]){
                                        node.changeMode(kV);
                                    }else{
            ...
            panel.addWidget( "combo", "Mode", HeavyGraph.NODE_MODES[node.mode], {values: HeavyGraph.NODE_MODES}, fUpdate);
	...
	LGraphCanvas.prototype.checkPanels = function()
	{
		if(!this.canvas)
			return;
		var panels = this.canvas.parentNode.querySelectorAll(".heavygraph.dialog");
    ...
    LGraphCanvas.onMenuNodeMode = function(value, options, e, menu, node) {
        new HeavyGraph.ContextMenu(
            HeavyGraph.NODE_MODES,
        ...
        function inner_clicked(v) {
            ...
            var kV = Object.values(HeavyGraph.NODE_MODES).indexOf(v);
            var fApplyMultiNode = function(node){
				if (kV>=0 && HeavyGraph.NODE_MODES[kV])
					node.changeMode(kV);
				else{
					console.warn("unexpected mode: "+v);
					node.changeMode(HeavyGraph.ALWAYS);
				}
			}
    ...
    LGraphCanvas.onMenuNodeColors = function(value, options, e, menu, node) {
        ...
        new HeavyGraph.ContextMenu(values, {
            event: e,
            callback: inner_clicked,
            parentMenu: menu,
            node: node
    ...
    LGraphCanvas.onMenuNodeShapes = function(value, options, e, menu, node) {
        ...
        new HeavyGraph.ContextMenu(HeavyGraph.VALID_SHAPES, {
            event: e,
            callback: inner_clicked,
            parentMenu: menu,
            node: node
    ...
    LGraphCanvas.onMenuNodeToSubgraph = function(value, options, e, menu, node) {
		...
		var subgraph_node = HeavyGraph.createNode("graph/subgraph");
    ...
    LGraphCanvas.prototype.getCanvasMenuOptions = function() {
        ...
        if (this.getMenuOptions) {
            ...
            /*if (HeavyGraph.showCanvasOptions){
    ...
    LGraphCanvas.prototype.processContextMenu = function(node, event) {
        ...
        if (slot) {
            ...
            if (slot.input && slot.input.type == HeavyGraph.ACTION) {
                options.title = "Action";
            }
            if (slot.output && slot.output.type == HeavyGraph.EVENT) {
                options.title = "Event";
            }
        } else {
            if (node) {
        ...
        var menu = new HeavyGraph.ContextMenu(menu_info, options, ref_window);
    ...
    HeavyGraph.compareObjects = compareObjects;
    ...
    HeavyGraph.colorToString = colorToString;
    ...
    HeavyGraph.isInsideRectangle = isInsideRectangle;
    ...
    HeavyGraph.growBounding = growBounding;
    ...
    HeavyGraph.isInsideBounding = isInsideBounding;
    ...
    HeavyGraph.overlapBounding = overlapBounding;
    ...
    HeavyGraph.num2hex = num2hex;

    /* HeavyGraph GUI elements used for canvas editing *************************************/
    ...
    function ContextMenu(values, options) {
        ...
        var root = document.createElement("div");
        root.className = "heavygraph litecontextmenu litemenubar-panel";
		...
		HeavyGraph.pointerListenerAdd(root,"up",
            function(e) {
        ...
        HeavyGraph.pointerListenerAdd(root,"down",
            function(e) {
        ...
        /*HeavyGraph.pointerListenerAdd(root,"leave", function(e) {
		...
		HeavyGraph.pointerListenerAdd(root,"enter", function(e) {
    ...
    ContextMenu.prototype.addItem = function(name, value, options) {
        ...
        if (!disabled && options.autoopen) {
			HeavyGraph.pointerListenerAdd(element,"enter",inner_over);
        }
    ...
    ContextMenu.prototype.close = function(e, ignore_parent_menu) {
        ...
        if (this.parentMenu && !ignore_parent_menu) {
            ...
            if (e === undefined) {
                ...
                ContextMenu.trigger(this.parentMenu.root, HeavyGraph.pointerevents_method+"leave", e);
            }
        }
        ...
        // TODO implement : HeavyGraph.contextMenuClosed(); :: keep track of opened / closed / current ContextMenu
    ...
    HeavyGraph.ContextMenu = ContextMenu;

    HeavyGraph.closeAllContextMenus = function(ref_window) {
    ...
    HeavyGraph.extendClass = function(target, origin) {
        for (var i in origin) {
	...
	HeavyGraph.CurveEditor = CurveEditor;

    //used to create nodes from wrapping functions
    HeavyGraph.getParameterNames = function(func) {
        return (func + "")
	...
	HeavyGraph.pointerListenerAdd = function(oDOM, sEvIn, fCall, capture=false) {
		if (!oDOM || !oDOM.addEventListener || !sEvIn || typeof fCall!=="function"){
			//console.log("cant pointerListenerAdd "+oDOM+", "+sEvent+", "+fCall);
			return; // -- break --
		}
		
		var sMethod = HeavyGraph.pointerevents_method;
		var sEvent = sEvIn;
	...
	HeavyGraph.pointerListenerRemove = function(oDOM, sEvent, fCall, capture=false) {
		...
		switch(sEvent){
			...
			case "down": case "up": case "move": case "over": case "out": case "enter":
			{
				if (HeavyGraph.pointerevents_method=="pointer" || HeavyGraph.pointerevents_method=="mouse"){
					oDOM.removeEventListener(HeavyGraph.pointerevents_method+sEvent, fCall, capture);
				}
			}
			// only pointerevents
			case "leave": case "cancel": case "gotpointercapture": case "lostpointercapture":
			{
				if (HeavyGraph.pointerevents_method=="pointer"){
					return oDOM.removeEventListener(HeavyGraph.pointerevents_method+sEvent, fCall, capture);
				}
			}
			// not "pointer" || "mouse"
			default:
...
if (typeof exports != "undefined") {
    exports.HeavyGraph = this.HeavyGraph;
