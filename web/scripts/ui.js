...
import { ExlrDialog as _ExlrDialog } from "./ui/dialog.js";
import { toggleSwitch } from "./ui/toggleSwitch.js";
import { ExlrSettingsDialog } from "./ui/settings.js";

export const ExlrDialog = _ExlrDialog;
...
function dragElement(dragEl, settings) {
	...
	function ensureInBounds() {
		if (dragEl.classList.contains("exlr-menu-manual-pos")) {
	...
	function restorePos() {
		let pos = localStorage.getItem("Exlr.MenuPosition");
		if (pos) {
			pos = JSON.parse(pos);
			newPosX = pos.x;
			newPosY = pos.y;
	...
	function elementDrag(e) {
		...
		dragEl.classList.add("exlr-menu-manual-pos");
...
export class ExlrUI {
	constructor(app) {
		this.app = app;
		this.dialog = new ExlrDialog();
		this.settings = new ExlrSettingsDialog(app);

		this.batchCount = 1;
		this.lastQueueSize = 0;
		this.queue = new exlrList("Queue");
		this.history = new exlrList("History", "history", true);
		...
		this.settings.addSetting({
			id: "Exlr.FloatRoundingPrecision",
		...
		this.menuContainer = $el("div.exlr-menu", {parent: document.body}, [
			// $el("span", {$: (q) => (this.queueSize = q)}),
			$el("button.exlr-queue-btn", {
				id: "queue-button",
				textContent: "Queue Prompt",
				onclick: () => app.queuePrompt(0, this.batchCount),
			}),
			$el("button.exlr-settings-btn", {textContent: "settings", onclick: () => this.settings.show()}),
			// $el("div", {}, [
			...
			$el("button", {id: "exlr-load-button", textContent: "Load", onclick: () => fileInput.click()}),
			$el("button", {
				id: "exlr-refresh-button",
				textContent: "Refresh",
				onclick: () => app.refreshComboInNodes()
			}),
			$el("button", {id: "exlr-clipspace-button", textContent: "Clipspace", onclick: () => app.openClipspace()}),
			$el("button", {
				id: "exlr-clear-button", textContent: "Clear", onclick: () => {
			...
			$el("button", {
				id: "exlr-load-default-button", textContent: "Load Default", onclick: async () => {
		...
		const devMode = this.settings.addSetting({
			...
			type: "boolean",
			defaultValue: false,
			onChange: function(value) { document.getElementById("exlr-dev-save-api-button").style.display = value ? "block" : "none"},
		});
