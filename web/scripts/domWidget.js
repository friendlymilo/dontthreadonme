	...
	if (selectedNode && selectedNode !== node) {
		...
		const intersection = intersect(
			{ x: elRect.x / scale, y: elRect.y / scale, width: elRect.width / scale, height: elRect.height / scale },
			{
				...
				y: selectedNode.pos[1] + app.canvas.ds.offset[1] - HeavyGraph.NODE_TITLE_HEIGHT - MARGIN,
...
function computeSize(size) {
	...
	for (const w of this.widgets) {
		if (w.type === "converted-widget") {
			...
			let minHeight = w.options.getMinHeight?.() ?? parseInt(styles.getPropertyValue("--exlr-widget-min-height"));
			let maxHeight = w.options.getMaxHeight?.() ?? parseInt(styles.getPropertyValue("--exlr-widget-max-height"));

			let prefHeight = w.options.getHeight?.() ?? styles.getPropertyValue("--exlr-widget-height");
			...
			widgetHeight += HeavyGraph.NODE_WIDGET_HEIGHT + 4;
		}
	}
	...
	for (const w of this.widgets) {
		...
		if (w.computedHeight) {
			...
			y += HeavyGraph.NODE_WIDGET_HEIGHT + 4;
		}
	}
}
