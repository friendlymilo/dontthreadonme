import { ExlrLogging } from "./logging.js";
import { ExlrWidgets, initWidgets } from "./widgets.js";
import { ExlrUI, $el } from "./ui.js";
...
export const ANIM_PREVIEW_WIDGET = "$$exlr_animation_preview"
...
/**
 * @typedef {import("types/exlr").exlrExtension} exlrExtension
 */

export class ExlrApp {
	/**
	...
	constructor() {
		this.ui = new ExlrUI(this);
		this.logging = new ExlrLogging(this);
	...
	getPreviewFormatParam() {
		let preview_format = this.ui.settings.getSettingValue("Exlr.PreviewFormat");
		if(preview_format)
	...
	static onClipspaceEditorSave() {
		if(ExlrApp.clipspace_return_node) {
			ExlrApp.pasteFromClipspace(ExlrApp.clipspace_return_node);
		}
	}

	static onClipspaceEditorClosed() {
		ExlrApp.clipspace_return_node = null;
	}
	...
	static copyToClipspace(node) {
		...
		if(ExlrApp.clipspace_invalidate_handler) {
			ExlrApp.clipspace_invalidate_handler();
		}
	}

	static pasteFromClipspace(node) {
		if(ExlrApp.clipspace) {
			// image paste
			if(ExlrApp.clipspace.imgs && node.imgs) {
				if(node.images && ExlrApp.clipspace.images) {
					if(ExlrApp.clipspace['img_paste_mode'] == 'selected') {
						node.images = [ExlrApp.clipspace.images[ExlrApp.clipspace['selectedIndex']]];
					}
					else {
						node.images = ExlrApp.clipspace.images;
					}
				...
				if(ExlrApp.clipspace.imgs) {
					// deep-copy to cut link with clipspace
					if(ExlrApp.clipspace['img_paste_mode'] == 'selected') {
						const img = new Image();
						img.src = ExlrApp.clipspace.imgs[ExlrApp.clipspace['selectedIndex']].src;
						node.imgs = [img];
						node.imageIndex = 0;
					}
					else {
						const imgs = [];
						for(let i=0; i<ExlrApp.clipspace.imgs.length; i++) {
							imgs[i] = new Image();
							imgs[i].src = ExlrApp.clipspace.imgs[i].src;
							node.imgs = imgs;
						}
					}
				}
			...
			if(node.widgets) {
				if(ExlrApp.clipspace.images) {
					const clip_image = ExlrApp.clipspace.images[ExlrApp.clipspace['selectedIndex']];
				...
				if(ExlrApp.clipspace.widgets) {
					ExlrApp.clipspace.widgets.forEach(({ type, name, value }) => {
	 ...
	 * @param {keyof exlrExtension} method The extension callback to execute
		...
		node.prototype.getExtraMenuOptions = function (_, options) {
			...
			if (!ExlrApp.clipspace_return_node) {
				options.push({
					...
					callback: (obj) => {
						ExlrApp.copyToClipspace(this);
					},
				});

				if (ExlrApp.clipspace != null) {
					options.push({
						content: "Paste (Clipspace)",
						callback: () => {
							ExlrApp.pasteFromClipspace(this);
						},
					});
				}

				if (ExlrApp.isImageNode(this)) {
					options.push({
						content: "Open in MaskEditor",
						callback: (obj) => {
							ExlrApp.copyToClipspace(this);
							ExlrApp.clipspace_return_node = this;
							ExlrApp.open_maskeditor();
						},
					});
				}
			}
			...
			if (node.imageOffset != null) {
				...
				if (node.widgets?.length) {
					...
					if (w.computeSize) {
						...
						shiftY += HeavyGraph.NODE_WIDGET_HEIGHT + 4;
					}
				} else {
				...
				if (this.imgs?.length) {
					...
					if (imageIndex == null) {
						...
						for (let i = 0; i < numImages; i++) {
							...
							if (!anyHovered) {
								anyHovered = HeavyGraph.isInsideRectangle(
									mouse[0],
									mouse[1],
									x + this.pos[0],
									y + this.pos[1],
						...
						const drawButton = (x, y, sz, text) => {
							const hovered = HeavyGraph.isInsideRectangle(mouse[0], mouse[1], x + this.pos[0], y + this.pos[1], sz, sz);
							let fill = "#333";
							let textFill = "#fff";
			...
			for (const item of items) {
				if (item.type.startsWith('image/')) {
					...
					if (this.canvas.current_node &&
						...
						ExlrApp.isImageNode(this.canvas.current_node)) {
						imageNode = this.canvas.current_node;
					}

					// No image node selected: add a new one
					if (!imageNode) {
						const newNode = HeavyGraph.createNode("LoadImage");
	...
	#addProcessMouseHandler() {
		...
		LGraphCanvas.prototype.processMouseDown = function(e) {
			...
			if (this.selected_group && !this.selected_group_resizing) {
				var font_size =
					this.selected_group.font_size || HeavyGraph.DEFAULT_GROUP_FONT_SIZE;
				var height = font_size * 1.4;

				// Move group by header
				if (HeavyGraph.isInsideRectangle(e.canvasX, e.canvasY, this.selected_group.pos[0], this.selected_group.pos[1], this.selected_group.size[0], height)) {
	...
	#addDrawGroupsHandler() {
		...
		LGraphCanvas.prototype.drawGroups = function(canvas, ctx) {
			...
			for (var i = 0; i < groups.length; ++i) {
				...
				if (!HeavyGraph.overlapBounding(this.visible_area, group._bounding)) {
					continue;
	...
	#addDrawNodeHandler() {
		...
		LGraphCanvas.prototype.drawNodeShape = function (node, ctx, size, fgcolor, bgcolor, selected, mouse_over) {
			...
			if (color) {
				const shape = node._shape || node.constructor.shape || HeavyGraph.ROUND_SHAPE;
				ctx.lineWidth = lineWidth;
				ctx.globalAlpha = 0.8;
				ctx.beginPath();
				if (shape == HeavyGraph.BOX_SHAPE)
					ctx.rect(-6, -6 - HeavyGraph.NODE_TITLE_HEIGHT, 12 + size[0] + 1, 12 + size[1] + HeavyGraph.NODE_TITLE_HEIGHT);
				else if (shape == HeavyGraph.ROUND_SHAPE || (shape == HeavyGraph.CARD_SHAPE && node.flags.collapsed))
					ctx.roundRect(
						-6,
						-6 - HeavyGraph.NODE_TITLE_HEIGHT,
						12 + size[0] + 1,
						12 + size[1] + HeavyGraph.NODE_TITLE_HEIGHT,
						this.round_radius * 2
					);
	...
	async #loadExtensions() {
	    ...
	    this.logging.addEntry("Exlr.App", "debug", { Extensions: extensions });
	...
	async #migrateSettings() {
		...
		const settings = Object.keys(this.ui.settings).reduce((p, n) => {
			const v = localStorage[`Exlr.Settings.${n}`];
			if (v) {
				try {
					p[n] = JSON.parse(v);
				} catch (error) {}
	...
	async #setUser() {
		...
		let user = localStorage["Exlr.userId"];
		...
		if (!user || !users[user]) {
			...
			user = userId;
			localStorage["Exlr.userName"] = username;
			localStorage["Exlr.userId"] = user;
		...
		this.ui.settings.addSetting({
			...
			name: "Switch User",
			type: (name) => {
				let currentUser = localStorage["Exlr.userName"];
				if (currentUser) {
				...
				return $el("tr", [
					...
					$el("td", [
						$el("button", {
							...
							onclick: () => {
								delete localStorage["Exlr.userId"];
								delete localStorage["Exlr.userName"];
	...
	async setup() {
		...
		// Create and mount the HeavyGraph in the DOM
		...
		HeavyGraph.release_link_on_empty_shows_menu = true;
		HeavyGraph.alt_drag_do_clone_nodes = true;

		this.graph.start();

		function resizeCanvas() {
			// Limit minimal scale to 1, see https://github.com/exlranonymous/ExlrUI/pull/845
	...
	async registerNodeDef(nodeId, nodeData) {
		...
		const node = Object.assign(
			function exlrNode() {
				...
				for (const o in nodeData["output"]) {
					...
					const outputShape = nodeData["output_is_list"][o] ? HeavyGraph.GRID_SHAPE : HeavyGraph.CIRCLE_SHAPE ;
			...
			},
			{
				...
				exlrClass: nodeData.name,
				nodeData
			}
		);
		node.prototype.exlrClass = nodeData.name;
		...
		HeavyGraph.registerNodeType(nodeId, node);
	...
	showMissingNodesError(missingNodeTypes, hasAddedNodes = true) {
		...
		this.logging.addEntry("Exlr.App", "warn", {
	...
	async loadGraphData(graphData, clean = true) {
		...
		for (let n of graphData.nodes) {
			...
			if (!(n.type in HeavyGraph.registered_node_types)) {
	...
	async graphToPrompt() {
		...
		for (const outerNode of this.graph.computeExecutionOrder(false)) {
			...
			for (const node of innerNodes) {
				...
				let node_data = {
					inputs,
					class_type: node.exlrClass,
				};

				if (this.ui.settings.getSettingValue("Exlr.DevMode")) {
	...
	loadApiJson(apiData) {
		const missingNodeTypes = Object.values(apiData).filter((n) => !HeavyGraph.registered_node_types[n.class_type]);
		if (missingNodeTypes.length) {
		...
		for (const id of ids) {
			...
			const node = HeavyGraph.createNode(data.class_type);
	...
	/**
	 * Registers a Exlr web extension with the app
	 * @param {exlrExtension} extension
	 */
