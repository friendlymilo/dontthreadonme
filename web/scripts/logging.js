import { $el, ExlrDialog } from "./ui.js";
...
class exlrLoggingDialog extends ExlrDialog {
	constructor(logging) {
		super();
	...
	export() {
		...
		const a = $el("a", {
			href: url,
			download: `exlrui-logs-${Date.now()}.json`,
			style: { display: "none" },
			parent: document.body,
		});
		a.click();
	...
	show(entries) {
		...
		const headers = Object.values(cols).map((title) =>
			$el("div.exlr-logging-title", {
				textContent: title,
			})
		);
		const rows = entries.map((entry, i) => {
			return $el(
				"div.exlr-logging-log",
				{
...
export class ExlrLogging {
	...
	constructor(app) {
		...
		this.dialog = new exlrLoggingDialog(this);
		this.addSetting();
		this.catchUnhandled();
		this.addInitData();
	}
	...
	addSetting() {
		const settingId = "Exlr.Logging.Enabled";
