...
export function createImageHost(node) {
	const el = $el("div.exlr-img-preview");
	let currentImgs;
	let first = true;
	...
	function updateSize() {
		...
		if (currentImgs) {
			...
			if (first) {
				...
				el.style.setProperty("--exlr-widget-min-height", elH);
			} else {
				el.style.setProperty("--exlr-widget-min-height", null);
			}
			...
			el.style.setProperty("--exlr-img-preview-width", w);
			el.style.setProperty("--exlr-img-preview-height", h);
		}
	}
	return {
		el,
