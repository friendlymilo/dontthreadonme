...
export function toggleSwitch(name, items, { onChange } = {}) {
	...
	function updateSelected(index) {
		if (selectedIndex != null) {
			elements[selectedIndex].classList.remove("exlr-toggle-selected");
		}
		onChange?.({ item: items[index], prev: selectedIndex == null ? undefined : items[selectedIndex] });
		selectedIndex = index;
		elements[selectedIndex].classList.add("exlr-toggle-selected");
	}
	...
	const container = $el("div.exlr-toggle-switch", elements);
