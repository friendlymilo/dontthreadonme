...
import { ExlrDialog } from "./dialog.js";

export class ExlrSettingsDialog extends ExlrDialog {
	constructor(app) {
		super();
		this.app = app;
		this.settingsValues = {};
		...
		this.element = $el(
			"dialog",
			{
				id: "exlr-settings-dialog",
				parent: document.body,
			},
			[
				$el("table.exlr-modal-content.exlr-table", [
	...
	async setSettingValueAsync(id, value) {
		...
		localStorage["Exlr.Settings." + id] = json; // backwards compatibility for extensions keep setting in storage
	...
	addSetting({ id, name, type, defaultValue, onChange, attrs = {}, tooltip = "", options = undefined }) {
		...
		if (value == null) {
			if (this.app.isNewUserSession) {
				...
				const localValue = localStorage["Exlr.Settings." + id];
				if (localValue) {
		...
		this.settingsLookup[id] = {
			...
			render: () => {
				...
				const labelCell = $el("td", [
					$el("label", {
						for: htmlID,
						classList: [tooltip !== "" ? "exlr-tooltip-indicator" : ""],
						textContent: name,
					}),
				]);
