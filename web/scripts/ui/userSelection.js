...
export class UserSelectionScreen {
	async show(users, user) {
		...
		const userSelection = document.getElementById("exlr-user-selection");
		...
		return new Promise((resolve) => {
			...
			const error = userSelection.getElementsByClassName("exlr-user-error")[0];
			const button = userSelection.getElementsByClassName("exlr-user-button-next")[0];
			...
			if (user) {
				const name = localStorage["exlr.userName"];
				if (name) {
					input.value = name;
				}
			}
