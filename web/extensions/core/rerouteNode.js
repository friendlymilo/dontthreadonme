	...
	registerCustomNodes(app) {
		class RerouteNode {
			constructor() {
				...
				this.onAfterGraphConfigured = function () {
					requestAnimationFrame(() => {
						this.onConnectionsChange(HeavyGraph.INPUT, null, true, null);
					});
				};
				...
				this.onConnectionsChange = function (type, index, connected, link_info) {
					...
					if (connected && type === HeavyGraph.OUTPUT) {
			...
			computeSize() {
				return [
					this.properties.showOutputText && this.outputs && this.outputs.length
						? Math.max(75, HeavyGraph.NODE_TEXT_SIZE * this.outputs[0].name.length * 0.6 + 40)
						: 75,
					26,
				];
			}
			...
			static setDefaultTextVisibility(visible) {
				...
				if (visible) {
					localStorage["Exlr.RerouteNode.DefaultVisibility"] = "true";
				} else {
					delete localStorage["Exlr.RerouteNode.DefaultVisibility"];
				}
			}
		}

		// Load default visibility
		RerouteNode.setDefaultTextVisibility(!!localStorage["Exlr.RerouteNode.DefaultVisibility"]);

		HeavyGraph.registerNodeType(
			"Reroute",
			Object.assign(RerouteNode, {
				title_mode: HeavyGraph.NO_TITLE,
				title: "Reroute",
				collapsable: false,
			})
		);
