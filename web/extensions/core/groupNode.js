	...
	static async registerFromWorkflow(groupNodes, missingNodeTypes) {
		...
		app.clean = function () {
			for (const g in groupNodes) {
				try {
					HeavyGraph.unregisterNodeType("workflow/" + g);
				} catch (error) {}
			}
			app.clean = clean;
		};
...
export class GroupNodeHandler {
	...
	constructor(node) {
			...
			if (!this.innerNodes) {
				this.node.setInnerNodes(
					this.groupData.nodeData.nodes.map((n, i) => {
						const innerNode = HeavyGraph.createNode(n.type);
		...
		this.node.recreate = async () => {
			...
			const groupNode = HeavyGraph.createNode(this.node.type);
			groupNode.id = id;
		...
		this.node.onDrawTitleBox = function (ctx, height, size, scale) {
			...
			ctx.fillStyle = this.boxcolor || HeavyGraph.NODE_DEFAULT_BOXCOLOR;
			ctx.fill();
			ctx.fillStyle = fill;
		};
		...
		node.onDrawForeground = function (ctx) {
			...
			if (+app.runningNodeId === this.id && this.runningInternalNodeId !== null) {
				...
				ctx.fillStyle = node.boxcolor || HeavyGraph.NODE_DEFAULT_BOXCOLOR;
				ctx.beginPath();
				ctx.roundRect(0, -HeavyGraph.NODE_TITLE_HEIGHT - 20, sz.width + 12, 20, 5);
				ctx.fill();

				ctx.fillStyle = "#fff";
				ctx.fillText(message, 6, -HeavyGraph.NODE_TITLE_HEIGHT - 6);
				ctx.restore();
			}
		};
	...
	static async fromNodes(nodes) {
		...
		const groupNode = HeavyGraph.createNode(`workflow/${name}`);
		// Reuse the existing nodes for this instance
