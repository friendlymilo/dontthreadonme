import { ExlrWidgets, addValueControlWidgets } from "../../scripts/widgets.js";
...
function convertToInput(node, widget, config) {
	...
	for (const widget of node.widgets) {
		widget.last_y += HeavyGraph.NODE_SLOT_HEIGHT;
	}
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		...
		nodeType.prototype.onInputDblClick = function (slot) {
			...
			if (!input.widget || !input[ignoreDblClick]) {
				...
				if (!(input.type in ExlrWidgets) && !(input.widget[GET_CONFIG]?.()?.[0] instanceof Array)) {
					return r; //also Not a ExlrWidgets input or combo (do nothing)
				}
			}

			// Create a primitive node
			const node = HeavyGraph.createNode("PrimitiveNode");
			app.graph.add(node);

			// Calculate a position that wont directly overlap another node
			const pos = [this.pos[0] - node.size[0] - 30, this.pos[1]];
			while (isNodeAtPos(pos)) {
				pos[1] += HeavyGraph.NODE_TITLE_HEIGHT;
			}
	...
	registerCustomNodes() {
		...
		class PrimitiveNode {
			...
			onConnectOutput(slot, type, input, target_node, target_slot) {
				...
				if (!input.widget) {
					if (!(input.type in ExlrWidgets)) return false;
				}
			...
			#onFirstConnection(recreating) {
				...
				let widget;
				if (!input.widget) {
					if (!(input.type in ExlrWidgets)) return;
			...
			#createWidget(inputData, node, widgetName, recreating, targetWidget) {
				...
				if (type in ExlrWidgets) {
					widget = (ExlrWidgets[type](this, "value", inputData, app) || {}).widget;
				} else {
