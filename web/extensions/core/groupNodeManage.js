import { $el, ExlrDialog } from "../../scripts/ui.js";
...
export class ManageGroupDialog extends ExlrDialog {
	...
	constructor(app) {
		super();
		this.app = app;
		this.element = $el("dialog.exlr-group-manage", {
	...
	getGroupData() {
		this.groupNodeType = HeavyGraph.registered_node_types["workflow/" + this.selectedGroup];
	...
	show(type) {
		...
		this.innerNodesList = $el("ul.exlr-group-manage-list-items");
		this.widgetsPage = $el("section.exlr-group-manage-node-page");
		this.inputsPage = $el("section.exlr-group-manage-node-page");
		this.outputsPage = $el("section.exlr-group-manage-node-page");
		...
		const outer = $el("div.exlr-group-manage-outer", [
			$el("header", [
				$el("h2", "Group Nodes"),
				$el(
					"select",
			...
			$el("main", [
				$el("section.exlr-group-manage-list", this.innerNodesList),
				$el("section.exlr-group-manage-node", [
					$el(
						"header",
			...
			$el("footer", [
					...
					"button.exlr-btn",
					{
						onclick: (e) => {
							...
							if (confirm(`Are you sure you want to remove the node: "${this.selectedGroup}"`)) {
								...
								HeavyGraph.unregisterNodeType("workflow/" + this.selectedGroup);
							}
							this.show();
						},
					},
				...
				$el("button.exlr-btn", { onclick: () => this.element.close() }, "Close"),
			]),
		]);
