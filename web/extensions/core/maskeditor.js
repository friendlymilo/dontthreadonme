...
import { ExlrDialog, $el } from "../../scripts/ui.js";
import { ExlrApp } from "../../scripts/app.js";
...
async function uploadMask(filepath, formData) {
	...
	ExlrApp.clipspace.imgs[ExlrApp.clipspace['selectedIndex']] = new Image();
	ExlrApp.clipspace.imgs[ExlrApp.clipspace['selectedIndex']].src = api.apiURL("/view?" + new URLSearchParams(filepath).toString() + app.getPreviewFormatParam() + app.getRandParam());

	if(ExlrApp.clipspace.images)
		ExlrApp.clipspace.images[ExlrApp.clipspace['selectedIndex']] = filepath;
...
class MaskEditorDialog extends ExlrDialog {
	...
	createLeftSlider(self, name, callback) {
		...
		divElement.style.backgroundColor = "var(--exlr-input-bg)";
			...
			mutations.forEach(function(mutation) {
					if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
						if(self.last_display_style && self.last_display_style != 'none' && self.element.style.display == 'none') {
							...
							ExlrApp.onClipspaceEditorClosed();
						}
	...
	async setImages(imgCanvas) {
		...
		// image load
		const filepath = ExlrApp.clipspace.images;

		const alpha_url = new URL(ExlrApp.clipspace.imgs[ExlrApp.clipspace['selectedIndex']].src)
		...
		// original image load
		const rgb_url = new URL(ExlrApp.clipspace.imgs[ExlrApp.clipspace['selectedIndex']].src);
	...
	async save() {
		...
		if(ExlrApp.clipspace.images)
			ExlrApp.clipspace.images[0] = item;

		if(ExlrApp.clipspace.widgets) {
			const index = ExlrApp.clipspace.widgets.findIndex(obj => obj.name === 'image');

			if(index >= 0)
				ExlrApp.clipspace.widgets[index].value = item;
		}
		...
		ExlrApp.onClipspaceEditorSave();
		this.close();
	}
}

app.registerExtension({
	name: "Exlr.MaskEditor",
	init(app) {
		...
		const context_predicate = () => ExlrApp.clipspace && ExlrApp.clipspace.imgs && ExlrApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("MaskEditor", context_predicate, ExlrApp.open_maskeditor);
	}
});
